...
{if $view_mode == "single_button"}
    ...
    <input class="cm-picker-value-description {$extra_class}"
           ...
           size="10" name="user_name"
           readonly="readonly" {$extra}>

    {include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="profiles.picker_admins?display=`$display`&picker_for=`$picker_for`&extra=`$extra_var`&checkbox_name=`$checkbox_name`&root=`$default_name`&except_id=`$except_id`&data_id=`$data_id``$extra_url`"|fn_url but_role="text" but_icon="icon-plus" but_target_id="content_`$data_id`" but_meta="`$but_meta` cm-dialog-opener add-on btn"}
